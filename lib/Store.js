...
function sanitizeSession(raw) {
  ...
  // Whether bufferText held real unsaved edits when this was written —
  // NOT just "does it differ from disk". A session written right after a
  // clean save must record dirty:false, so a later restore never lets a
  // stale-but-clean buffer outrank newer disk content (e.g. the same
  // Dropbox-synced document edited from a different machine in between).
  // Missing/old-format sessions (before this field existed) default to
  // false — the safe direction, since it means "trust disk", never
  // "silently overwrite disk with a maybe-stale buffer".
  var dirty = (raw && typeof raw.dirty === "boolean") ? raw.dirty : false
  return { lastDocPath: lastDocPath, bufferText: bufferText, dirty: dirty }
}
